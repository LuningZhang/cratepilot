...
from PySide6.QtWidgets import QHBoxLayout, QLabel, QLineEdit, QMainWindow, QMessageBox, QPushButton, QSplitter, QVBoxLayout, QWidget
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, root_folder: str, session_factory: sessionmaker):
        ...
        self.reveal_button.clicked.connect(self.reveal_selected_location)
    ...
    def run_scan(self) -> None:
        ...
        QMessageBox.information(
            self, "Scan complete", f"Scanned: {result.scanned_files}\nImported: {result.imported_tracks}\nFailed: {result.failed_files}"
        )
    ...
    def bulk_edit(self) -> None:
        ...
        if dialog.exec():
            ...
            if not updates:
                return
            if (
                QMessageBox.question(self, "Confirm bulk edit", f"Apply {len(updates)} fields to {len(self.selected_tracks)} tracks?")
    ...
    def closeEvent(self, event):  # noqa: N802
        if self.watch_handle is not None: self.watch_handle.stop()
        self.watch_handle = None
